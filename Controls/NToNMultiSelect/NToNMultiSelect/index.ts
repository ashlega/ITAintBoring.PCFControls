...
export class NToNMultiSelect implements ComponentFramework.StandardControl<IInputs, IOutputs> {
  ...
  public init(context: ComponentFramework.Context<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container: HTMLDivElement) {
    ...
    if (typeof Xrm == "undefined") {
      ...
      this.btnNode.classList.add("pcf_btn_element");
      this.btnNode.onclick = this.openNewForm.bind(this);
      this.mainContainer.appendChild(this.btnNode);
      ...
      var newOption = new Option("+ Add New", "", false, false);
      var thisVar: any;
      thisVar = this;
      $(document).ready(function () {
        ...
        $("#" + thisVar._ctrlId)
          .select2({
            closeOnSelect: false,
            matcher: (searchTerm, optionData) => {
              if (searchTerm.term == null || searchTerm.term == "") {
                return optionData;
              }
              if (optionData.text == "+ Add New") {
                return optionData;
              } else if (optionData.text.toUpperCase().indexOf(searchTerm.term.toUpperCase()) >= 0) {
                return optionData;
              }
            ...
            templateResult: function (value) {
              if (value.id == "") {
                return $(`<div style="min-width: 100%;min-height: 20px;"><span style="float: right;font-weight:bold" class='select2-add-new'>${value.text}</span><div>`);
              }
              return value.text;
            },
            // language: {
            ...
            //     return `<li class="select2-results__option" id="select2-${thisVar._ctrlId}-result-jbg4- role="option" aria-selected="false" data-select2-id="select2-${thisVar._ctrlId}-result-jbg4-">+ Add New</li>`;
            //   },
            // },
  ...
  public openNewForm(): void {
    var entityFormOptions: ComponentFramework.NavigationApi.EntityFormOptions = {
      entityName: this._linkedEntityName,
      useQuickCreateForm: true,
    };
    this.contextObj.navigation.openForm(entityFormOptions).then(
      (success) => {
        if (success !== null) {
          this.destroy();
          this.init(this.contextObj, this._notifyOutputChanged, this.contextObj, this.container);
        }
      },
      (error) => {
        this.contextObj.navigation.openAlertDialog(error);
      }
    );
  }
  /**
